const users = [
  ...
  const analyzeUsers = (users) => {
    ...
    // Step 1: Filter active users who have posted in the last week
    const activeUsers = users.filter(user => 
        user.posts.some(post => new Date(post.timestamp) >= oneWeekAgo)
    );
  
    // Step 2: Extract popular posts (at least 10 likes)
    const popularPosts = activeUsers.flatMap(user => 
        user.posts.filter(post => new Date(post.timestamp) >= oneWeekAgo && post.likes >= 10)
    );
